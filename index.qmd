...
```{ojs}
{
  let b = benchmark_datasets.find(l => l.Name == dataset_to_be_visualized)["sparkline data"].replaceAll("'", '"')
  try {
    b = JSON.parse(b)
    return Plot.plot({
    color: {
    ...
    marks: [
      ...
      Plot.text(['Distribution of graph sizes by number of nodes in ' + dataset_to_be_visualized], {frameAnchor: "Top",dy: -20})
    ]
  })
...
<div id="named-list" data-bs-spy="scroll"  data-bs-target="db-nav-list" data-bs-offset="20" tabindex="0"></div>

```{ojs}
{
  let named_list_dom = d3.select("#named-list");
  named_list_dom.selectAll("*").remove()
  let entry =  benchmark_datasets.find(l => l.Name == dataset_to_be_visualized);
...
```{ojs}
function add_entry_row(named_list_dom, entry, paper_data, origin_paper_data, notionFolders){
  ...
  //add buttons
        ...
        let otherinfo = ""
        try{
            // TODO:Sara replace this for quarterly
            ...
            const location = entry["Name"].replace(".", "") + " " + entry["Page id"] + ".md";
            let otherinfo = notionFolders[location];
            // otherinfo = await otherinfo.text() //TODO:Sara you need to get the text
...
```{ojs}
//| echo: false
function make_dataset_name_and_title_list(chosen_collection_type){ 
  ...
  for (let c of [... new Set(collection_type)]){
    ...
    for (let i in instances){
      ...
      if (instances[i]["Originally found at"] != "" && instances[i]["Originally found at"] != undefined) 
        ...
        .attr("transform", "translate(410, " + (y_distance * i + 65) + ")")
        .attr("target", "_blank")
        .append("text")
        .text("[link]")
        .style("fill", "#88d")
      ...
      svg.append("a")
        .attr("href", instances[i]["Originally found at"])
        .attr("transform", "translate(470, " + (y_distance * i + 65) + ")")
        .attr("target", "_blank")
        .append("text")
