  ...
  let infocontainer = named_list_dom.append("div")
        ...
        for (let e of entry["Origin paper plaintext"].split(",")){
            ...
            if (origindoi != undefined && origindoi != ""){
                inforow.append("div").html('<a href="' + doify(origindoi) + '">' + "[link]" + '</a> &nbsp; <a style="cursor: pointer; text-decoration: underline; color: rgba(var(--bs-link-color-rgb)" onclick="citation(' + cite_index + ')">' + "" + '</a> &nbsp;' + e)
...
function name_cleanup(name){
  return name.replace("(", "").replace(")", "").replace(" ", "_").replace(".", "").replace("'", "").replace("&", "").replaceAll(" ", "").replaceAll(",", "")
}
```
